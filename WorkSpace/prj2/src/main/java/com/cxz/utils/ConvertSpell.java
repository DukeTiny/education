...
public class ConvertSpell {
    ...
    private static Hashtable<String,String> ht = new Hashtable(10);

    public static void main(String[] args) {
        String a = "为境外窃取剌探收买非法提供国家秘密情报罪";//nclm
        String result = convert(a);
        System.out.println(getSimple(a));
    }

    public static String getSimple(String word) {
        word = word.replaceAll("、", "");
        char[] chars = word.toCharArray();
        StringBuilder result = new StringBuilder();
        for (char c : chars) {
            String temp = convert(c + "");
            if (temp.length() > 0)
                result.append(convert(c + "").substring(0, 1));
        }
